...
def extract_endo_epi_flows(image):
    ...
    # Convert to RGB (for pytesseract and easyocr)
    image = image.convert("RGB")
    
    # Optionally save the master RGB image
    ...
    for i, box in enumerate(SECOND_COLUMN_BOXES, start=1):
        ...
        # Optionally save the cropped image with a unique filename
        cropped_filename = f"cropped_image_{i}"
        cropped_path = os.path.join(image_folder, cropped_filename)
        # cropped.save(cropped_path)

        # Perform OCR using the chosen engine
        if OCR_ENGINE == "pytesseract":
            text = pytesseract.image_to_string(cropped, config=CUSTOM_CONFIG).strip().replace(',', '.')
